...
// A roleta gira e para sempre 100% normal/aleatória (física intacta). A única
// coisa que muda é o NOME mostrado no overlay ao final: se o admin tiver
// definido um "vencedor pré-definido" no painel, esse nome substitui o nome
// que caiu de verdade na roleta. Se não houver nada definido, mostra quem
// realmente caiu (comportamento original).
function _obterVencedorForcado() {
...
function suave() {
  ...
  function step() {
    ...
    if (vel > 0) {
      ...
      const vReal = nomes[i];
      const nomeForcado = _obterVencedorForcado();
      const vExibido = nomeForcado || vReal;
